  ...
  <div class="article-container">
      ...
      <div class="article-detail" v-if="!!article.art_id">
        ...
        <van-cell class="user-info" center :border="false">
          ...
          <!-- <FollowUser :value="article.is_followed" @input="article.is_followed=$event"></FollowUser> -->
           <FollowUser v-model="article.is_followed" :target="article.aut_id"></FollowUser>
          <!-- <van-button
            class="follow-btn"
            round
            size="small"
        ...
        <!-- 文章内容 -->
        <div class="article-content markdown-body" v-html="article.content">这是文章内容</div>
        <van-divider>正文结束</van-divider>
      <ArticleComment :source="article.art_id" type="a" @set-count="count=$event" :commentList="commentList"
      @replay-show="comment=$event;isReplayShow=true;"></ArticleComment>
      </div>
      <!-- /加载完成-文章详情 -->
      <!-- 加载失败：404 -->
    ...
    <div class="article-bottom" v-if="!isLoading && !!article.art_id">
      <van-button class="comment-btn" type="default" round size="small"
         @click="addCommentShow=true">写评论</van-button
      >
...
/>
   <van-popup v-model="addCommentShow" position="bottom">
    <AddComment v-if="addCommentShow" :target="article_id"
     @add-comment="commentList.unshift($event);addCommentShow=false;"></AddComment>
   </van-popup>
...
export default {
  ...
  async created () { 
    ...
    // 在这一块的时候loading关闭了，页面显示正文，但是必须还要写在nextTick
    // 因为数据更新 视图不能立即更新 写在nextTick回调中可以  因为nextTick回调就是在视图更新完成后才会触发
    this.$nextTick(()=>{
      // 获取正文中所有图片
      const arr=document.querySelectorAll('.article-content img')
      if(arr.length===0) return    //防御性代码，代码更加安全
      const arr1=[]   //放图片的src
      // forEach遍历先把所有的图片src地址放在arr1里面
      arr.forEach((img,index)=>{
        console.log(img.src);
        arr1.push(img.src)
        // 给图片绑定点击事件点击的时候出现预览效果
