<template>
  <div class="user-profile">
    ...
    <van-cell title="昵称" :value="profile.name" is-link @click="isNameShow=true"/>
    <van-cell title="性别" :value="profile.gender===0 ? '男' : '女'" is-link />
    <van-cell title="生日" :value="profile.birthday" is-link @click="isBirthdayShow=true"/>
    <van-popup position="bottom" style="height:100%" v-model="isNameShow">
        <van-nav-bar title="更新昵称" left-text="取消" right-text="保存" @click-left="onNameCancel" @click-right="onNameSave"/>
        <van-field :value="profile.name" rows="2" autosize type="textarea" 
        maxlength="50" placeholder="请输入昵称" show-word-limit @input="name=$event"/>
    </van-popup>
...
/>
    ...
    <!-- 更新头像 -->
    <input type="file" style="display:none" accept="image/png,image/jpeg,image/jpg,image/gif" ref="file" @change="onChange"/>
